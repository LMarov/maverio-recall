...
- **Ask** — `POST /ask` (`src/routes/ask.ts`, `src/pipeline/ask.ts`) answers
  a free-text question about the team's meetings. When the question isn't
  scoped to one meeting, `src/pipeline/retrieval.ts` ranks every meeting by
  real relevance to the question — Postgres full-text search over title/
  objective/summary/decisions/actions/gaps (migration `0004`), not just the
  most recent ones — and tops up with recency for whatever's left when a
  question's wording doesn't overlap the corpus, so Ask never comes up
  empty on a real, if oddly-phrased, question. It hands Claude the relevant
  meeting content (summary/objective/decisions/actions/gaps for the ranked
  meetings, or one meeting's full transcript when the question is scoped to
  it) with the same "use only what's given" grounding as the analysis
  pipeline, and returns an answer plus citations back to the source
  meeting(s). Same API-key-stays-server-side rule as the rest of the
  pipeline.
- **Observability** — every request gets a `x-request-id` (echoed on the
  response, and on any error body), logged as structured JSON in production
  via `pino`/`pino-http` (`src/logger.ts`) instead of plain text. Any 4xx/5xx
  is also persisted to the `error_log` table (`src/errorLog.ts`, migration
  `0005`) and readable via `GET /admin/errors` (owners/admins only,
  `src/routes/admin.ts`) — so "what broke, for who, and when" survives past
  whatever's in the server's stdout scrollback. A crash outside a request
  (`uncaughtException`/`unhandledRejection`) is logged and persisted the same
  way before the process exits, so a process manager restart doesn't erase
  the reason it happened. `GET /health/ready` checks the database is
  actually reachable (not just that the process is up), and `GET /metrics`
  exposes Prometheus-format request counts/latencies plus default Node
  process metrics for anything that wants to scrape them.
...
| `NODE_ENV` | `production` switches logging to plain JSON lines (for a log shipper to parse); anything else pretty-prints to the console. `test` silences it entirely (Vitest's own output stays readable) |
| `LOG_LEVEL` | Minimum pino log level (default `info`, or `silent` when `NODE_ENV=test`) |
| `METRICS_TOKEN` | Optional — when set, `GET /metrics` requires `Authorization: Bearer <this>`. Leave unset for local dev; set it before exposing the server publicly |
...
- `src/logger.ts` — the app-wide `pino` logger (JSON in production, pretty
  in dev, silent in tests). `src/errorLog.ts` — persists/reads back captured
  errors (`error_log` table, migration `0005`). `src/metrics.ts` — the
  `prom-client` registry + custom HTTP counters/histogram behind `/metrics`.
...
- `src/routes/` — one file per resource (`auth`, `team`, `clients`,
  `meetings`, `scheduled`, `audio`, `ask`, `audit`, `admin`).
...
- `src/pipeline/` — `transcribe.ts` (Deepgram), `analyze.ts` (Claude),
  ...
  grounded in the team's meetings), `retrieval.ts` (ranks meetings by real
  relevance to the question via Postgres full-text search, falling back to
  recency), `voiceprint.ts` (Picovoice Eagle enroll/identify),
  `audioDecode.ts` (ffmpeg-static: compressed upload -> 16kHz mono PCM for
  Eagle).
...
- `rateLimit.ts`'s in-memory counters are per-process — fine for a single
  ...
  a load balancer.
- `/ask`'s retrieval is Postgres full-text search (see below), not vector/
  semantic search — it ranks on shared vocabulary, so a question phrased
  with completely different words than the meeting used (e.g. "runway" vs.
  "budget headroom") can still miss a real match, falling back to recency
  instead. Fine at the volume a team produces today; a genuinely semantic
  upgrade would mean embeddings + a vector column, a bigger lift than this
  phase's scope.
- Voiceprint enrollment isn't cumulative — naming a speaker rebuilds their
  ...
  confirmed in.
- Observability is self-contained (Postgres table + a scrapable endpoint),
  not wired to an external service (Sentry, Datadog, a hosted Prometheus,
  ...) — deliberately, so it works with zero extra accounts/config. Pointing
  `/metrics` at a real scraper or shipping the JSON logs somewhere durable is
  an infra step for whoever deploys this, not code left to write.
